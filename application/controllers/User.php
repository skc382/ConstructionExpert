...
class User extends CI_Controller {
        ...
        if ($this->form_validation->run() == TRUE) {
          if($this->db->insert('users',array(
            ...
            $this->email->message($content);
             if($this->email->send()){
               echo json_encode(["status"=>2,"msg"=>'Thank you for registering here. Verification link sent to your mail',"verificationlink"=>urlencode($href),"registerphone"=>$this->input->post('mobile')]);
             }
        }      
  ...
  public function serviceprovider_list($value='')
  {
    ...
    foreach($query->result() as $r) {
      ...
      ' <a href="#" class="btn btn-sm bg-danger-light delete-serviceprovider" title="Delete" id="'.$r->userid.'"><i class="fe fe-trash"></i>Delete</a>
  ...
  public function updatepassword(){
    ...
    public function delete_serviceprovider(){  
    if($this->db->delete('users',array('userid'=>$this->input->post('id')))){
