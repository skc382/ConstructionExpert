...
class Settings extends CI_Controller {
  ...
  public function update_site(){
    ...
    }else if(!empty($_FILES['hfile']['name'][0]))
    {
        // upload settings
        $config = array(
          'upload_path' => './backend/img/web/',
      ...
      if (!$this->upload->do_upload('hfile'))
      {
          // case - failure            
          echo json_encode(["status"=>0,"msg"=>$this->upload->display_errors()]);
      }
      else
      {
          ...
          // delete previous logo
          $img = $this->db->get_where('settings')->row();  
          $sourcimg = './backend/img/web/' .$img->headerlogo;
          // $thumbimg = './backend/img/web/thumb/'.$img->headerlogo;  
          unlink($sourcimg);
          // unlink($thumbimg);
          $about=$this->input->post('about');        
          $this->db->update('settings',array(
