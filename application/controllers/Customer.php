...
class Customer extends CI_Controller {
 ...
 public function delete_myad(){
     
    if($this->db->delete('jobads',array('adid'=>$this->uri->segment(2)))){
      redirect(BASE_URL('my-ads'),'refresh');
    }
  }
    ...
    foreach($query->result() as $r) {
      ...
      if($r->status == 1)  {        
        ...
        $status =  '<span class="text-danger">Inactive</span>';
          ...
          $action ='<div class="actions text-right"><a href="#" id='.$r->adid.' class="btn btn-sm bg-info viewad" title="View Enquiry Details">View</a>';
        if($r->status == 4) {
          $action .='<a href="#" class="btn btn-sm bg-warning" title="Invite Service Provider">Invite</a>';
        }else{
          $action .='<a href="'.base_url().'invite-service-provider/'.$r->adid.'" class="btn btn-sm bg-warning" title="Invite Service Provider">Invite</a>';
        }        
        $action .='<a href="'.base_url().'view-job-notification/'.$r->adid.'" class="btn btn-sm bg-info" title="View Job Request">Notification</a>
        </div>';
        $tab[] =$action;
       $data[] = $tab; 
         $sno=$sno+1;
  ...
  public function serviceprovider_list(){
    ...
    $data = [];  $sno = 1; 
    foreach($query->result() as $r) {
        $url = urlencode(base_url().'ad-details/'.$this->uri->segment(3));
     ...
     $img = '<div class="avatar '.$status.'">
      ...
      <a href="https://wa.me/'.$r->phone.'/?text=Check Job Enquiry '.$url.'" target="new" style="background-color:green;"
      class="btn btn-sm text-white" title="Invite via whatsapp"><i class="fa fa-whatsapp fa-fw"></i>Invite via whatsapp</a>';
       $data[] = $tab; 
         $sno=$sno+1;
