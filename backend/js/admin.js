...
if(page == 'location'){
  ...
  function load_location(){    
    $('#industry_table').DataTable({ 
        pageLength: 10,     
        ajax: base_url+"servicelocation/location_list",
        serverMethod: 'get',
        bFilter:false
    });
  }
  ...
  $(document).on('submit','#addTag',function(e){
    ...
    $.ajax({
      type:"post",
      url: base_url+"servicelocation/add_loc",
      data:$(this).serialize(),
      success:function(res){
        var rep = JSON.parse(res);
       if(rep.status == 1){
  ...
  $(document).on('click','#delloc',function(e){
    e.preventDefault();
    var id = $("#cid").val();
    $.ajax({
      type:"post",
      url: base_url+"servicelocation/delete_loc",
      data:{id:id, csrf_token_name:csrf_token},
      success:function(res){
        var rep = JSON.parse(res);
       if(rep.status == 1){
  ...
  $(document).on('click','.edit-loc',function(e){
    e.preventDefault();
    var id = $(this).attr('id');
    $.ajax({
      type:"post",
      url: base_url+"servicelocation/getTagbyid",
      data:{id:id, csrf_token_name:csrf_token},
      success:function(res){
        var rep = JSON.parse(res); 
        $("#etagid").val(rep.locid);
        $("#etagname").val(rep.location);          
        $("#edit_tag").modal('show');
      },error:function(xhr){
        console.log(xhr);
      }
  ...
  $(document).on('click','.delete-loc',function(e){
    e.preventDefault();
    var id = $(this).attr('id');
    $("#cid").val(id);
    $("#delete_modal").modal('show');
...
//srs
   ...
   if(page == 'srs'){       
     ...
     $(document).on('click','.delete-serviceprovider',function(e){
      e.preventDefault();
      var id = $(this).attr('id'); 
      $("#uid").val(id);
      $("#delete_modal").modal('show');
    ...
    $(document).on('click','#delsrsuser',function(e){
      e.preventDefault();
      var id = $("#uid").val();
      $.ajax({
        type:"post",
        url: base_url+"user/delete_serviceprovider",
        data:{id:id, csrf_token_name:csrf_token},
        success:function(res){
          var rep = JSON.parse(res);
         if(rep.status == 1){
...
if(page == 'invitesrs'){
  ...
  function load_srs(){
    ...
    $('#industry_table').DataTable({ 
      pageLength: 10,     
      ajax: base_url+"customer/serviceprovider_list/"+adid,
      serverMethod: 'get',
      bFilter:false
   });
   }
